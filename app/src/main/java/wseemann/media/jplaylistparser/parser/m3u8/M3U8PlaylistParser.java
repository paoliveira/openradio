...
public final class M3U8PlaylistParser extends AbstractParser {
    ...
    public M3U8PlaylistParser(final int timeout) {
        super(timeout);
    }
     ...
     * @throws IOException
     */
    private void parsePlaylist(final String uri, final InputStream stream, final Playlist playlist) throws IOException {
        final String host = getHost(uri);
        if (TextUtils.isEmpty(host)) {
            throw new IOException("Provided URI '" + uri + "' is invalid");
        }
    ...
    private void savePlaylistFile(final PlaylistEntry playlistEntry, final Playlist playlist) {
    ...
    private String getHost(final String uri) {
        ...
        try {
            ...
            if (index > -1) {
                host = new URI(
                        host.getScheme(), null, host.getHost(), host.getPort(), path.substring(0, index + 1),
                        null, null
                );
            }
    ...
    private String generateUri(final String uri, final String host) {
