...
class AbstractStorageTest {
    ...
    fun fromStringToList() {
        ...
        val list = AbstractRadioStationsStorage.getAllFromString(mContext!!, value)
    ...
    @Test
    fun getAllAddAll() {
        val storageName = "TestAbstractStorage"

        AbstractRadioStationsStorage.clear(mContext!!, storageName)

        val num = 10
        val list = mutableListOf<RadioStation>()
        for (i in 0..num) {
            list.add(RadioStationTest.makeTestInstance(mContext!!, "id-$i", i))
        }

        AbstractRadioStationsStorage.addAll(mContext!!, storageName, list)

        val newList = AbstractRadioStationsStorage.getAll(mContext!!, storageName)
