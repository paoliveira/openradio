...
class PackageValidator(context: Context, @XmlRes xmlResId: Int) {
    ...
    private val callerChecked = HashMap<String, Pair<Int, Boolean>>()
     ...
     * See [com.yuriy.openradio.shared.service.OpenRadioService.onGetRoot] for where this is utilized.
     *
    ...
    fun isKnownCaller(callingPackage: String, callingUid: Int): Boolean {
        ...
        if (callerPackageInfo == null) {
            AppLogger.e("Caller wasn't found in the system?")
            return false
        }

        // Verify that things aren't ... broken. (This test should always pass.)
        if (callerPackageInfo.uid != callingUid) {
            AppLogger.e("Caller's package UID doesn't match caller's actual UID?")
            return false
        }
    ...
    private fun buildCallerInfo(callingPackage: String): CallerPackageInfo? {
        ...
        val activePermissions = HashSet<String>()
    ...
    private fun buildCertificateAllowList(parser: XmlResourceParser): Map<String, KnownCallerInfo> {
        ...
        val certificateAllowList = HashMap<String, KnownCallerInfo>()
        try {
                ...
                if (eventType == XmlResourceParser.START_TAG) {
                    ...
                    callerInfo?.let { info ->
                        ...
                        if (existingCallerInfo != null) {
                            existingCallerInfo.signatures.addAll(callerInfo.signatures)
                        } else {
    ...
    private fun parseV1Tag(parser: XmlResourceParser): KnownCallerInfo {
        ...
        return KnownCallerInfo(name, packageName, mutableSetOf(callerSignature) as HashSet<KnownSignature>)
    }
    ...
    private fun parseV2Tag(parser: XmlResourceParser): KnownCallerInfo {
        ...
        val callerSignatures = HashSet<KnownSignature>()
        var eventType = parser.next()
     ...
     */
    private fun getSystemSignature() =
    ...
    private fun getSignatureSha256(certificate: ByteArray): String {
        ...
        try {
            ...
            AppLogger.e("Could not find SHA256 hash algorithm", noSuchAlgorithmException)
    ...
    private data class KnownCallerInfo(
        ...
        val signatures: HashSet<KnownSignature>
    )
